...
@section('content')
    <div id="mainContent" class="transition-all duration-300 ml-64 pt-[109px] md:pt-[61px] min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        <div class="container mx-auto px-6 py-8">
            ...
            <!-- Notifikasi Cepat -->
            <div class="bg-gradient-to-r from-yellow-400 to-orange-400 rounded-2xl p-6 mb-8 shadow-lg transform hover:scale-102 transition-all duration-300">
                <div class="flex items-center">
                    ...
                    <div class="flex-1">
                        <p class="font-bold text-white text-lg">{{ $statusDiajukan }} Pengajuan Baru!</p>
                        <p class="text-white/90">Ada pengajuan magang baru yang perlu ditinjau</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-3">
                        @foreach(\App\Models\PartnerModel::orderByDesc('created_at')->take(5)->get() as $mitra)
                            <div class="flex justify-between items-center py-3 px-4 bg-gray-50 rounded-xl hover:bg-purple-50 transition-all duration-300">
                                ...
                                <span class="text-xs text-gray-500 bg-white px-2 py-1 rounded-full">{{ $mitra->created_at->format('d M Y') }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        @foreach(\App\Models\LowonganModel::orderByDesc('created_at')->take(5)->get() as $lowongan)
                            <div class="flex justify-between items-center py-3 px-4 bg-gray-50 rounded-xl hover:bg-yellow-50 transition-all duration-300">
                                ...
                                <span class="text-xs text-gray-500 bg-white px-2 py-1 rounded-full">{{ $lowongan->created_at->format('d M Y') }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
