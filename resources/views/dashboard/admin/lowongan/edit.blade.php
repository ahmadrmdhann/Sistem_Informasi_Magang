...
@section('content')
    <div id="mainContent"
        ...
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold">Edit Lowongan</h2>
            ...
            <form action="{{ route('lowongan.update', $lowongan->lowongan_id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="partner_id" id="partner_id"
                        ...
                        @foreach($partners as $partner)
                            <option value="{{ $partner->partner_id }}" {{ $partner->partner_id == $lowongan->partner_id ? 'selected' : '' }}>{{ $partner->nama }}</option>
                        @endforeach
                    </select>
                    @error('partner_id')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="judul" id="judul"
                        ...
                        value="{{ old('judul', $lowongan->judul) }}" placeholder="Masukkan judul lowongan" required>
                    @error('judul')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="persyaratan" id="persyaratan"
                        ...
                        required>{{ old('persyaratan', $lowongan->persyaratan) }}</textarea>
                    @error('persyaratan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="lokasi" id="lokasi"
                        ...
                        value="{{ old('lokasi', $lowongan->lokasi) }}" placeholder="Masukkan lokasi" required>
                    @error('lokasi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="bidang_keahlian" id="bidang_keahlian"
                        ...
                        value="{{ old('bidang_keahlian', $lowongan->bidang_keahlian) }}"
                ...
                <div class="mb-4">
                    ...
                    <select name="periode_id" id="periode_id"
                        ...
                        @foreach($periodes as $periode)
                            <option value="{{ $periode->periode_id }}" {{ $periode->periode_id == $lowongan->periode_id ? 'selected' : '' }}>{{ $periode->nama }}</option>
                        @endforeach
                    </select>
                    @error('periode_id')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tanggal_akhir" id="tanggal_akhir"
                        ...
                        value="{{ old('tanggal_akhir', $lowongan->tanggal_akhir) }}" required>
                    @error('tanggal_akhir')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6 flex gap-2">
                    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-md font-medium">
