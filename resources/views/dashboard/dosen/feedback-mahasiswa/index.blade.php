...
@section('content')
    <div id="mainContent" class="transition-all duration-300 ml-64 pt-[109px] md:pt-[61px] min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        <div class="container mx-auto px-6 py-8">
            ...
            @if (session('success'))
                <div class="bg-gradient-to-r from-green-400 to-green-500 text-white p-6 rounded-2xl mb-8 shadow-lg">
            ...
            @if (session('error'))
                <div class="bg-gradient-to-r from-red-400 to-red-500 text-white p-6 rounded-2xl mb-8 shadow-lg">
                    <div class="flex items-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div class="group bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl p-6 border border-white/50 transform hover:-translate-y-2 transition-all duration-300">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <h4 class="font-bold text-2xl text-gray-800">{{ $stats['total_responses'] }}</h4>
                            <p class="text-blue-500 text-xs">Feedback</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="group bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl p-6 border border-white/50 transform hover:-translate-y-2 transition-all duration-300">
                    <div class="flex items-center">
                        ...
                        <div>
                            <p class="text-gray-500 text-sm font-medium">Total</p>
                            <h4 class="font-bold text-2xl text-gray-800">{{ $stats['total_students'] }}</h4>
                            <p class="text-green-500 text-xs">Mahasiswa</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="group bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl p-6 border border-white/50 transform hover:-translate-y-2 transition-all duration-300">
                    <div class="flex items-center">
                        ...
                        <div>
                            <p class="text-gray-500 text-sm font-medium">Total</p>
                            <h4 class="font-bold text-2xl text-gray-800">{{ $stats['total_partners'] }}</h4>
                ...
                <div class="group bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl p-6 border border-white/50 transform hover:-translate-y-2 transition-all duration-300">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-yellow-500 text-xs">Rating</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Filters -->
            <div class="bg-white/70 backdrop-blur-xl rounded-3xl shadow-xl border border-white/50 overflow-hidden mb-8">
                <div class="p-8">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6">Filter Feedback</h2>
                    
                    <form method="GET" action="{{ route('dosen.feedback-mahasiswa.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-6">
                        <div>
                            ...
                            <select name="mahasiswa_id" id="mahasiswa_id" 
                                    class="w-full px-4 py-2.5 border border-gray-300 rounded-xl bg-white/50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div class="md:col-span-5 flex space-x-3">
                            <button type="submit" 
                                    class="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 shadow-lg flex items-center">
                                <i class="fas fa-search mr-2"></i>
                                Filter
                            </button>
                            <a href="{{ route('dosen.feedback-mahasiswa.index') }}" 
                               class="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50 shadow-lg flex items-center">
                    ...
                    @else
                        <div class="space-y-6">
                            @foreach($feedbackResponses as $response)
                                <div class="bg-white/60 backdrop-blur-sm border border-white/60 rounded-2xl p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                                    <div class="flex justify-between items-start mb-4">
                                        <div class="flex-1">
                                            <div class="flex items-center space-x-4 mb-3">
                                                ...
                                                <div>
                                                    <h3 class="text-lg font-bold text-gray-800">{{ $response->mahasiswa->user->nama }}</h3>
                                    ...
                                    <!-- Quick Stats -->
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-white/60">
                                        ...
                                        <div class="text-center transform hover:scale-105 transition-transform duration-200">
                                            <div class="text-lg font-bold text-blue-600">{{ $response->answers->whereNotNull('rating_value')->count() }}</div>
                                            <div class="text-sm text-gray-600">Rating Questions</div>
                                        </div>
                                        <div class="text-center transform hover:scale-105 transition-transform duration-200">
                                            <div class="text-lg font-bold text-green-600">{{ $response->answers->whereNotNull('answer_text')->count() }}</div>
                                            <div class="text-sm text-gray-600">Text Questions</div>
                                        </div>
                                        <div class="text-center transform hover:scale-105 transition-transform duration-200">
                                            @if($avgRating)
                                                <div class="text-lg font-bold text-yellow-600">{{ number_format($avgRating, 1) }}</div>
                                    ...
                                    <div class="flex justify-end">
                                        <a href="{{ route('dosen.feedback-mahasiswa.show', $response->response_id) }}" 
                                           class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm rounded-xl font-medium transition-all duration-300 transform hover:scale-105 shadow-md">
